...
class FileDecisionBundleStore(DecisionBundleStorePort):
    ...
    def get_latest_pair(self) -> Optional[tuple[str, str]]:
        ...
        latest_as_of = common_as_ofs[-1]

        bt_file = bt_by_as_of[latest_as_of]
        run_file = run_by_as_of[latest_as_of]

        return str(bt_file), str(run_file)
    ...
    def load_artifact(self, artifact_id: str) -> RunArtifact:
        ...
        weights = self._extract_weights(payload)
    ...
    def _extract_weights(payload: dict) -> dict[str, float]:
        ...
        raise KeyError("No supported weights field found")
